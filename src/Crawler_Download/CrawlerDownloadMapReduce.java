...
public class CrawlerDownloadMapReduce extends Configured implements Tool {  
	private static String recordFilePathOfHtmlFilePath;
	private static String level="1";
	private static String HtmlFilePath=configure.HTMLFILESPATH+level+"/";
	private static String HtmlInfoFilePath=configure.HTMLFILESINFOPATH+level+configure.HTMLINFONAME;
	private static String urlFilePath=configure.URLFILESPATH+level+configure.URLNAME;;
	 public static class MapClass extends Mapper<LongWritable, Text, Text, IntWritable> {  
			private Text reduceInputUrl = new Text();
			private final static IntWritable one = new IntWritable(1);
	        // Map Method  
	        public void map(LongWritable key, Text mapInputUrl, Context context) throws IOException, InterruptedException{  
	        	StringTokenizer tokenizer = new StringTokenizer(mapInputUrl.toString());
	    ...
	    public static class Reduce extends Reducer<Text,IntWritable,NullWritable,NullWritable> {  
	        ...
	        // Reduce Method 
	        public void reduce(Text url, Iterable<IntWritable> values, Context context) 
	        	...
	        	try {
					pageBody=(byte[])threadPool.submit(new CrawlerThread(url.toString(),HtmlFilePath)).get();
	        	...
	        	if(pageBody!=null){
	        		...
	        		String temp=
	        		DownloadPage.saveToHdfs(pageBody,url.toString(), HtmlFilePath,HtmlInfoFilePath);
	        		pathList.add(temp);
	        	}
	        	else{
	        		System.out.println("pagebody is null");
	        	}
	        		//threadPool.execute(new CrawlerThread(pageBody,url.toString(),HtmlFilePath));
	        		//threadPool.execute(new CrawlerThread(url.toString(),HtmlFilePath));
	        ...
	        protected void cleanup(Context context) throws IOException, InterruptedException {
			    ...
			    if (fileSystem.exists(new Path(HtmlInfoFilePath))) {
					System.out.println("File " + HtmlInfoFilePath + " already exists");
				    return;
			    }
			    FSDataOutputStream out = fileSystem.create(new Path(HtmlInfoFilePath));
	    ...
	    public static void main(String[] args) throws Exception {  
	        int res = ToolRunner.run(new Configuration(),new CrawlerDownloadMapReduce(), args);  
	        //System.exit(res);  
	    }  
...
/*
	public static class Reduce extends MapReduceBase
		...
		private final static Text test=new Text("test");
          ...
          OutputCollector<Text, Text> output, Reporter reporter) throws IOException{
			  ...
			  output.collect(url, test);
			  //new CrawlerThread(url.toString(),HtmlFilePath);
			  //}
				//if(reduceInputUrl.toString() != null && reduceInputUrl.toString().length() > 0){

			  //output.collect(url, NullWritable.get());
		  }
	}
	public static void main(String[] args)throws Exception{ 	
		HtmlFilePath=configure.HTMLFILESPATH+level+"/";
		String urlFilePath=configure.URLFILESPATH+level+configure.URLNAME;
		String HTMLInfoFilePath=configure.HTMLFILESINFOPATH+level;
